...
@Injectable()
export class ArticleCommentsInfoService extends BaseCrudService<ArticleCommentsInfo> {
   ...
   */
  getArticlesWithNewCommentsInfo(daysAgo: number): Observable<ArticleCommentsInfo[]>{
    const options = daysAgo ?
    { params: new HttpParams().set('daysAgo', daysAgo.toString()) } : {}
    return this.httpClient.get<ArticleCommentsInfo[]>(`${this.url}/GetArticlesWithNewCommentsInfo`, options)
}
...
public getWithPagAndSort(
  ...
  //         .set() } 
  console.log('exec the overrided method');
  return this.http
